...
import { insert, set, setIfMissing, useFormValue } from "sanity";

export function GenerateVariants(props) {
  ...
  const variantOptions = useFormValue(["options"]);
  ...
  const generateVariantName = (variantOptions) => {
  ...
  const handleClick = useCallback(() => {
    ...
    const generateVariantKey = (variantOptions) => {
      return (
        variantOptions
          .map((option) => `${option.name}:${option.value}`)
          .join("|") +
